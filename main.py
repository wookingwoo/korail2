...
def attempt_reservation():
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            # 잔여 좌석이 없을 때의 에러 처리
            print(f"[시도 {attempt}/{MAX_RETRIES}] 잔여 좌석이 없습니다.", end=" ")

        except SoldOutError:
            # 매진 에러 처리
            print(f"[시도 {attempt}/{MAX_RETRIES}] 매진되었습니다.", end=" ")

        except Exception as e:
            send_slack_message(f"기타 에러 발생: {e}", end=" ")
            break
    ...
    print(f"최대 {MAX_RETRIES}번 시도했으나 좌석 예약에 실패했습니다.")
    send_slack_message(
        f":man-gesturing-no: 최대 {MAX_RETRIES}번 시도했으나 좌석 예약에 실패했습니다."
    )
    return False
